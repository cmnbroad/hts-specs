...
The CRAM specification distinguishes between \textit{logical} data types and \textit{storage}
data types; \textit{storage} data types define how \textit{logical} data types are stored by CRAM.
Logical data types are referred to using names that are words (e.g. Integer), while physical data
types are referred to by single letters (e.g. i) (is this accurate ?). Data in a CRAM stream is
stored either as inidividual bits, or as bytes. Writing values as bits and bytes is described
in detail below.
...
CRAM uses \emph{little endianness} for bytes when applicable.

\subsection{\textbf{Storage Data Types}}

CRAM defines the following storage data types:
...
Encoding is a data structure that captures information about how logical data
types in a data series are encoded, compressed, and serialized. An encoding
might contain a set of constants that are required to initialize a specific
decompression algorithm, statistical properties of a data series or, in the
case of a data series being stored in an external block, the block content id. 

Encoding notation is defined as the keyword `encoding' followed by its logical
data type in angular brackets, for example `encoding\texttt{<}byte\texttt{>}'
stands for an encoding that operates on a data series of logical data type `byte'. 
...
compressed block. Only gzip is allowed as compression method for this block. The
BAM header is terminated with \textbackslash{}0 byte and any extra bytes in the
...
to reserve 50\% more space in the BAM container than is required by the
BAM header.
...
use a content id greater than 0. 
...
ltf8 & record counter & 1-based sequential index of records in the file/stream\tabularnewline
\hline
itf8 & number of blocks & the number of blocks in the slice, including the core block\tabularnewline
\hline
